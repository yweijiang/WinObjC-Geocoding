...
static inline __CGSurfaceInfo _CGSurfaceInfoInit(unsigned int width,
    ...
    surfaceInfo.pixelProperties.colorSpaceModel = c_FormatTable[fmt].colorSpaceModel;
    surfaceInfo.pixelProperties.bitsPerComponent = c_FormatTable[fmt].bitsPerComponent;
    surfaceInfo.pixelProperties.bytesPerPixel = c_FormatTable[fmt].bytesPerPixel;
    ...
    if (bitmapInfo != c_kCGBitmapInfoInvalidBits) {
        surfaceInfo.pixelProperties.bitmapInfo = bitmapInfo;
    } else {
        surfaceInfo.pixelProperties.bitmapInfo = c_FormatTable[fmt].bitmapInfo;
    }

    // Set bytesPerRow if necessary
    if ((data != NULL) && (bytesPerRow == 0)) {
        surfaceInfo.bytesPerRow = width * surfaceInfo.pixelProperties.bytesPerPixel;
    }
