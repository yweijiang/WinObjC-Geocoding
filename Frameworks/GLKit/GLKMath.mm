...
GLKIT_EXPORT GLKMatrix3 GLKMatrix3InvertAndTranspose(GLKMatrix3 m, BOOL* isInvertible) {
    ...
    const float a2 = m.m[3];
    const float b2 = m.m[4];
    const float c2 = m.m[5];
    const float a3 = m.m[6];
    const float b3 = m.m[7];
    const float c3 = m.m[8];
    ...
    if (determinant == 0) {
        ...
        res.m[7] = (m.m02 * m.m10 - m.m00 * m.m12) / determinant;
        res.m[8] = (m.m00 * m.m11 - m.m01 * m.m10) / determinant;
    }
...
GLKMatrix4 GLKMatrix4RotateWithVector3(GLKMatrix4 matrix, float radians, GLKVector3 axisVector) {
    GLKMatrix4 r = GLKMatrix4MakeRotation(radians, axisVector.x, axisVector.y, axisVector.z);
    return GLKMatrix4Multiply(matrix, r);
}
...
GLKMatrix4 GLKMatrix4ScaleWithVector3(GLKMatrix4 matrix, GLKVector3 scaleVector) {
    GLKMatrix4 scaleMatrix = GLKMatrix4MakeScale(scaleVector.x, scaleVector.y, scaleVector.z);
    return GLKMatrix4Multiply(matrix, scaleMatrix);
}
...
GLKMatrix4 GLKMatrix4TranslateWithVector3(GLKMatrix4 matrix, GLKVector3 translationVector) {
    GLKMatrix4 trans = GLKMatrix4MakeTranslation(translationVector.x, translationVector.y, translationVector.z);
    GLKMatrix4 res = GLKMatrix4Multiply(matrix, trans);
    return res;
}
